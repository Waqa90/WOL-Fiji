...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!hasAdminAccess && branchIds.length > 0) {
      query += ` AND mr.branch_id = ANY($${params.length + 1}::uuid[])`
      params.push(branchIds)
    }

    if (scheduleType) {
      query += ` AND mr.schedule_type = $${params.length + 1}`
      params.push(scheduleType)
    }

    if (scheduleDate) {
      query += ` AND mr.schedule_date = $${params.length + 1}`
      params.push(scheduleDate)
    }

    if (branchId) {
      query += ` AND mr.branch_id = $${params.length + 1}`
      params.push(branchId)
    }

    query += ` ORDER BY mr.schedule_date DESC, mr.schedule_type`

    const rosters = await sql(query, params)
